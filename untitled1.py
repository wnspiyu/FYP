...
Original file is located at
    https://colab.research.google.com/drive/1MaiWfmrWAn4fiJO6aIu2T6thWI6s3uvN
"""
import pandas as pd
...
#_______________________________________________________________________________-
# fig1 = plt.figure(figsize=(8, 6))
# Y1 = np.array([Ea,Eb,Ec,Ed])
# mylabels = ["Solar", "BESS", "HVO", "Biomass Boiler"]
# myexplode = [0, 0, 0, 0]   # To give space between parts
...
#Net Present values for new systems
# fig5 = plt.figure(figsize=(8, 6))
# plt.title("Net Present Value for New Systems")
# X5 = np.array(["Solar PV", "BESS", "Green Diesel DG", "Biomass Boiler", "Total System"])

# a = -objective([x[0],0,0,0])
# b = -objective([0,x[1],0,0])
# c = -objective([0,0,x[2],0])
# d = -objective([0,0,0,x[3]])

# Y5 = np.array([a,b,c,d,sum([a,b,c,d])])
#y = np.array([ -objective([x[0],0,0,0]), -objective([0,x[1],0,0]), -objective([0,0,x[2],0]), -objective([0,0,0,x[3]]),sum([ -objective([x[0],0,0,0]), -objective([0,x[1],0,0]), -objective([0,0,x[2],0]), -objective([0,0,0,x[3]]) ]) ])

# Add text labels to the bars (show values on the bars)
# for i in range(len(X5)):
#     plt.text(i, Y5[i], str(round(Y5[i],2)), ha='center', va='bottom')
...
# Cashflow of HVO Fuel generator
            # Optimized result of diesel generator in litres
# Multiply each element by the annnual HVO requirement using list comprehension
